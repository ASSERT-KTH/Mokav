...
class TestGenerator:
    ...
    def generate_test2(self, buggy_code, accepted_code, existing_test=None, retry=False):
        ...
        if retry:
            ...
            self.prompt_history.append((prompt, chatgpt_resp))
        else:
            ...
            if self.config == 'BA':
                ...
                chatgpt_resp = self.chatgpt.get_response(new_question=prompt)
            self.prompt_history.append((prompt, chatgpt_resp))
        print("###CHATRESP###", chatgpt_resp)
        return self.extract_code_blocks(str(chatgpt_resp))
